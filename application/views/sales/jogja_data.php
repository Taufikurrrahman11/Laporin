...
<section class="content">
  <div class="box">
    <div class="box-header">
      ...
      <!-- Date Filter Form -->
      <form action="<?= site_url('jogja/index') ?>" method="GET" class="form-inline">
    ...
    <div class="box-body table-responsive">
      <table class="table table-bordered table-striped" id="table1">
        <thead>
          <tr>
            <th>
                <a href="<?= site_url('jogja/index?sort_by=summary_id&order='.($order == 'asc' ? 'desc' : 'asc').'&start_date='.urlencode($start_date).'&end_date='.urlencode($end_date)) ?>">
                    <i class="fa <?= ($sort_by == 'summary_id' && $order == 'asc') ? 'fa-sort-amount-asc' : 'fa-sort-amount-desc' ?>"></i>
                </a>
              Sales ID 
            </th>
            <th>
                <a href="<?= site_url('jogja/index?sort_by=date&order='.($order == 'asc' ? 'desc' : 'asc').'&start_date='.urlencode($start_date).'&end_date='.urlencode($end_date)) ?>">
                  <i class="fa <?= ($sort_by == 'date' && $order == 'asc') ? 'fa-sort-amount-asc' : 'fa-sort-amount-desc' ?>"></i>
                </a>
              Date 
            </th>
            <th>
                <a href="<?= site_url('jogja/index?sort_by=sales_name&order='.($order == 'asc' ? 'desc' : 'asc').'&start_date='.urlencode($start_date).'&end_date='.urlencode($end_date)) ?>">
                  <i class="fa <?= ($sort_by == 'sales_name' && $order == 'asc') ? 'fa-sort-amount-asc' : 'fa-sort-amount-desc' ?>"></i>
                </a>
              Sales Name 
            </th>
            <th>
                <a href="<?= site_url('jogja/index?sort_by=email&order='.($order == 'asc' ? 'desc' : 'asc').'&start_date='.urlencode($start_date).'&end_date='.urlencode($end_date)) ?>">
                  <i class="fa <?= ($sort_by == 'email' && $order == 'asc') ? 'fa-sort-amount-asc' : 'fa-sort-amount-desc' ?>"></i>
                </a>
              Sales Email 
            </th>
            <th>
                <a href="<?= site_url('jogja/index?sort_by=total_register&order='.($order == 'asc' ? 'desc' : 'asc').'&start_date='.urlencode($start_date).'&end_date='.urlencode($end_date)) ?>">
                  <i class="fa <?= ($sort_by == 'total_register' && $order == 'asc') ? 'fa-sort-amount-asc' : 'fa-sort-amount-desc' ?>"></i>
                </a>
              Total Register 
            </th>
            <th>
                <a href="<?= site_url('jogja/index?sort_by=total_subscribe&order='.($order == 'asc' ? 'desc' : 'asc').'&start_date='.urlencode($start_date).'&end_date='.urlencode($end_date)) ?>">
